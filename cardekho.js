...
(async function fn(){
 ...
 let cpage=page;
 await cpage.goto(`https://www.cardekho.com/new-${inp[2]}+cars+${inp[0]}-lakh-${inp[1]}-lakh`);



 await cpage.waitFor(3000);
 let count = await cpage.$("#model-highlight");//problem here

if(count==null){
    count=await cpage.$("span[class='chips']");
}

 let loopCount= await cpage.evaluate(function cb(ele) {
 ...
 loop=loopCount.split(" ")[2];
   }

if(loop>=10){
    let Loopcount=Math.ceil(loop/10);
...
ansCars.push({name:cars[i].name,mileage:cars[i].mileage,engine:cars[i].engine,price:cars[i].price});
}
...
function writeInFile(cars,type){
